...
function bootTerminal() {
    ...
    if(btnStart) btnStart.onclick = () => { 
        ...
        window.ativarModoImersivo(); // 👈 A INJEÇÃO DA TELA CHEIA VOLTOU AQUI!
...
/* =========================================================
   SUBSTITUA A FUNÇÃO checkGameOver NO SEU SCRIPT.JS
   ========================================================= */
function checkGameOver() { 
    if (playerLife > 0 && enemyLife > 0) return; // Se ninguém morreu, segue o jogo

    gameIsOver = true; 
    clearInterval(timerInterval);
    isSystemLocked = true; // Trava tudo para o show

    const playerWon = playerLife > 0;
    const modal = document.getElementById("game-over-modal");
    
    // Configurações Dramáticas
    const finalColor = playerWon ? "#00ff00" : "#ff0000"; // Verde ou Vermelho
    const finalTitle = playerWon ? "SISTEMA HACKEADO" : "FALHA CRÍTICA";
    const finalSubtitle = playerWon ? "Acesso Nível Ômega Concedido." : "Conexão Neural Interrompida.";
    
    // GIFs Cyberpunk (Placeholders)
    // Vitória: Uma cidade futurista acendendo / Derrota: Uma tela com glitch e ruído
    const gifUrl = playerWon 
        ? "https://i.pinimg.com/originals/03/95/34/0395349bcc0db6334319650be9574588.gif" 
        : "https://i.gifer.com/7h3.gif"; 

    // 1. Dispara a Explosão Visual e o Flash de Tela
    if(typeof VFX !== 'undefined') VFX.bigExplosion(finalColor);

    // 2. Toca o Som da Vitória/Derrota (Opcional - usando os que já temos)
    playSound(playerWon ? "deploy" : "error"); 

    // 3. Monta o HTML Dramático
    modal.innerHTML = `
        <div class="modal-content final-screen" style="color: ${finalColor}">
            <img src="${gifUrl}" class="final-gif">
            <h1 class="final-title">${finalTitle}</h1>
            <p style="color: #ccc; margin-top: 10px; letter-spacing: 2px;">${finalSubtitle}</p>
            <button class="cmd-btn" id="btn-soft-reset" style="margin-top:30px; border-color: ${finalColor}; color: ${finalColor}; height: 60px; font-size: 1.2rem;">
                [ RETORNAR À BASE ]
            </button>
        </div>
    `;
    
    // Adiciona um delay dramático antes de mostrar o modal
    setTimeout(() => {
        ...
        document.getElementById("btn-soft-reset").onclick = () => {
            ...
            if(playerWon) { 
                playerFragments += 100; 
                document.getElementById("fragment-count").innerText = playerFragments;
                alert("Recompensa de Missão: +100 💽 CRÉDITOS");
            }
        };
