...
// =========================================================// =========================================================
// 💥 TEXTO DE DANO COM ANIMAÇÃO DE IMPACTO
// =========================================================
function mostrarTextoFlutuanteV2(alvoDOM, texto, cor = "#ff0055", isCritical = false) {
    if (!alvoDOM) return;
    
    const floatText = document.createElement("div");
    floatText.innerText = texto;
    
    const scale = isCritical ? 2.5 : 1.8;
    const duration = isCritical ? 1.5 : 1.2;
    ...
    floatText.style.cssText = `
        ...
        font-weight: bold;
        text-shadow: 0 0 20px ${cor}, 0 0 40px #000;
        z-index: 99999;
        pointer-events: none;
        white-space: nowrap;
        filter: blur(0.5px);
    ...
    // Animação GSAP com bounce e fade
    const tl = gsap.timeline();
    tl.fromTo(floatText, 
        { y: 0, opacity: 1, scale: 0.3, rotation: -15 }, 
        { y: -80, opacity: 0, scale: scale, rotation: 15, duration: duration, ease: "back.out(1.5)" }
    );
    
    // Efeito de tremer na carta ao tomar dano
    if(!isCritical) {
        gsap.to(alvoDOM, {
            x: "+=5",
            duration: 0.05,
            yoyo: true,
            repeat: 5,
            onComplete: () => gsap.set(alvoDOM, { x: 0 })
        });
    } else {
        // Critical hit - shake mais forte
        gsap.to(alvoDOM, {
            x: "+=10",
            rotation: "+=5",
            duration: 0.05,
            yoyo: true,
            repeat: 8,
            onComplete: () => gsap.set(alvoDOM, { x: 0, rotation: 0 })
        });
    }
    
    setTimeout(() => floatText.remove(), duration * 1000);
}
// =========================================================
...
// =========================================================
// 🎚️ MESA DE SOM (SISTEMA DE RÁDIO INTELIGENTE)
// =========================================================

// 1. Músicas do Menu (Tocam aleatoriamente na Loja, Deck, etc)
const menuPlaylist = [
    "./audios/som1.mp3",
    "./audios/som2.mp3",
     "./audios/som3.mp3",
...
// 2. Músicas Específicas das Arenas (O mapa exato do seu CSS)
const arenaMusic = {
    "theme-war": "./audios/war.mp3",
    "theme-lab": "./audios/labi.mp3",
    "theme-void": "./audios/vontex.mp3"
};

// 3. O Toca-Fitas Global
let currentBGM = new Audio();
currentBGM.loop = true;
currentBGM.volume = 0.15; 

// 4. Os Efeitos Especiais (SFX)
const sfx = { 
    click: new Audio("./audios/clique.wav"), 
    deploy: new Audio("./audios/invocar.wav"), 
    hit: new Audio("./audios/dano.wav"), 
    error: new Audio("./audios/erro.mp3") 
};
sfx.click.volume = 0.4; sfx.deploy.volume = 0.5; sfx.hit.volume = 0.6; sfx.error.volume = 0.3;  

// 5. O Cérebro do Áudio
function playSound(type, arenaClass = null) { 
    if (type === 'bgm') {
        currentBGM.pause(); 
        
        // Se a função receber o nome da arena, toca a música épica dela!
        if (arenaClass && arenaMusic[arenaClass]) {
            currentBGM.src = arenaMusic[arenaClass];
        } 
        // Se não recebeu nome de arena (está no Menu), roda a roleta aleatória
        else {
            let randomIndex = Math.floor(Math.random() * menuPlaylist.length); 
            currentBGM.src = menuPlaylist[randomIndex]; 
        }
...
const baseDeck = [
   ...
   // { title: "Hack de Sobrecarga", tipo: "feitico", raridade: "rara", custo: 3, atk: 0, def: 0, efeito: "dano 4", img: "https://files.catbox.moe/tl6rvy.png", som_drop: "https://files.catbox.moe/9h871i.wav" },
    ...
    // ==========================================
    { title: "Operativo Vácuo", tipo: "humano", raridade: "rara", custo: 3, atk: 3, def: 2, efeito: "silencio_aleatorio", text: "Grito de Guerra: Silencia uma tropa inimiga aleatória, anulando os seus efeitos.", img: "./vacuo.png" },
    { title: "Unidade Hazmat", tipo: "humano", raridade: "epica", custo: 4, atk: 2, def: 6, efeito: "aprisionar_slot", text: "Aprisiona um slot. Cartas nele não podem atacar enquanto esta unidade viver.", img: "./hazmat.png" },
    ...
    { title: "Drone Sombra", tipo: "automato", raridade: "comum", custo: 1, atk: 1, def: 1, efeito: "taxa_automato", text: "Aura: Enquanto estiver em campo, os Autômatos inimigos custam +1.", img: "./drone_sombra.png" },
...
// ==========================================
// 🧠 3. NÚCLEO DE MEMÓRIA (VARIÁVEIS GLOBAIS)
// ==========================================

// 👤 SETOR 1: PERFIL E INVENTÁRIO (Persistentes)
let playerName = "OPERADOR"; 
let playerAvatar = "./padrao.png"; 
let playerFragments = 70;
let currentLevel = 0;
let playerCollection = [];
let customDeck = [];

// 🃏 SETOR 2: A MESA DE JOGO (Cards em Partida)
let playerDeck = [];
let graveyard = { player: [], enemy: [] };

// ❤️ SETOR 3: STATUS DA BATALHA (Atributos Físicos)
let maxLife = 20, playerLife = 20, enemyLife = 20;
let maxMana = 3, playerMana = 3;
let currentTurn = 1;
let playerFatigue = 1; // 💀 Contador de Morte Súbita
let sobrecargaAtiva = { player: 0, enemy: 0 }; // Memória dos reatores

// ⚔️ SETOR 4: CONTROLE DE FLUXO (O Ping-Pong da Matrix)
let attackToken = "player"; // Quem tem a faca na mão (player ou enemy)
let currentStep = "deploy_attacker"; // Estágios: deploy_attacker -> deploy_defender -> combat
let isPlayerTurn = true; 
let gameIsOver = false; 
let isSystemLocked = false; // Trava a mesa durante animações da IA ou Feitiços

// 🖱️ SETOR 5: INTERAÇÃO E UI (O que o jogador está segurando)
let selectedAttacker = null; 
let selectedCardFromHand = null; 
let draggedCard = null; 
let selectedHeroObj = null; 
let currentSortMode = "cost"; // Filtro da loja/deck

// ⏱️ SETOR 6: MOTORES DE TEMPO
let turnTime = 80; 
...
// =========================================================
// 🚀 SISTEMA DE PROJÉTEIS 3D (Integração Three.js + GSAP)
// =========================================================
let projectileSystem = {
    activeProjectiles: [],
    
    createProjectile(fromCard, toCard, type = "energy", onHit = null) {
        // Obtém posições dos elementos DOM
        const fromRect = fromCard.getBoundingClientRect();
        const toRect = toCard.getBoundingClientRect();
        
        // Cria elemento visual do projétil
        ...
        // Animação GSAP com MotionPath
        const tl = gsap.timeline({
            onComplete: () => {
                projectile.remove();
                if(onHit) onHit(toCard);
                
                // Efeito de impacto
                this.createImpactEffect(toRect);
            }
        });
        ...
        tl.to(projectile, {
            ...
            motionPath: {
                path: [
                    {x: parseFloat(projectile.style.left), y: parseFloat(projectile.style.top)},
                    {x: (parseFloat(projectile.style.left) + toRect.left)/2, y: parseFloat(projectile.style.top) - 50},
    ...
    cleanup() {
        this.activeProjectiles = this.activeProjectiles.filter(p => p.parentElement);
    }
};
// ==========================================
// 4. INICIALIZAÇÃO
...
/// =========================================================
// ⚡ CLIQUE NO HERÓI (CORREÇÃO DE ATRIBUTO E BLINDAGEM)
// =========================================================
function handleHeroClick(heroElement, owner) {
    ...
    // 🛑 DIAGNÓSTICO ZERO: A carta perdeu a seleção antes de bater?
    if (owner === "enemy" && !selectedAttacker) {
        playSound("error");
        alert("SISTEMA: Selecione uma tropa sua primeiro antes de atacar a Ameaça!");
        return;
    }
    
    // ⚔️ SE O JOGADOR SELECIONOU UMA TROPA E CLICOU NO INIMIGO:
    if (selectedAttacker && owner === "enemy") {
        
        // 🛑 DIAGNÓSTICO 1: A Fase está correta?
        if (currentStep !== "combat") {
            playSound("error");
            if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(heroElement, "FORA DA FASE DE COMBATE", "#ffcc00");
            limparMiras();
            return;
        }

        // 🛑 DIAGNÓSTICO 2: O jogador tem o Token de Ataque?
        if (attackToken !== "player") {
            playSound("error");
            if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(heroElement, "TURNO INIMIGO", "#ffcc00");
            limparMiras();
            return;
        }

        // --- A TRAVA (FIREWALL ABSOLUTO) ---
        const campoInimigo = document.getElementById("enemy-field").querySelectorAll('.card-base');
        const defensores = Array.from(campoInimigo).filter(carta => carta.dataset.dead !== "true");
        ...
        // ==========================================
        // 💥 O ATAQUE DIRETO (Caminho Livre!)
        // ==========================================
        
        // ⚡ CORREÇÃO CRÍTICA AQUI: O atributo correto é "attack" e não "atk"!
        let danoDoAtacante = parseInt(selectedAttacker.dataset.attack) || 0;
        
        enemyLife -= danoDoAtacante; // Subtrai na memória
        if (enemyLife < 0) enemyLife = 0; // Não deixa a vida ficar negativa
        
        // ⚡ ATUALIZAÇÃO FORÇADA DE TELA 
        let enemyLifeDOM = document.getElementById("enemy-life");
        if (enemyLifeDOM) enemyLifeDOM.innerText = enemyLife;
        if (typeof updateLifeAndMana === "function") updateLifeAndMana();
        
        // Efeitos de Impacto
        if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(heroElement, `-${danoDoAtacante} HP`, "#ff0000");
        playSound("hit");
        ...
        if (typeof registrarLog === "function") registrarLog(`[${selectedAttacker.dataset.name}] atacou a AMEAÇA (-${danoDoAtacante} HP)`, "player");
...
// =========================================================
// 🔮 MOTOR DE FEITIÇOS E MAGIAS (EXECUTESPELL)
// =========================================================
function executeSpell(spellCard, targetElement, targetOwner) {
    ...
    // ⚡ BLINDAGEM 1: Define quem é o Player para as magias funcionarem!
    ...
    if (isLocalCaster) {
        ...
        playerMana -= cost; 
        if (typeof updateLifeAndMana === "function") updateLifeAndMana(); 
        if (typeof registrarLog === "function") registrarLog(`Lançou Magia: [${spellCard.dataset.name}]`, "player");
    ...
    // 💥 EFEITOS BASE DE DANO/CURA
    if(efeito === "dano_total" || efeito === "dano_area") { 
        document.getElementById(isLocalCaster ? "enemy-field" : "player-field").querySelectorAll('.card-base').forEach(inimigo => {
            if (typeof applyDamage === "function") applyDamage(inimigo, 3);
        }); 
        ...
        if(targetElement.id.includes("hero")) { 
            if(targetElement.id === "enemy-hero") enemyLife -= dmg; else playerLife -= dmg; 
            if(window.VFX && window.VFX.triggerTechBits) VFX.triggerTechBits(targetElement, "#ff0000"); 
        } else { 
            if (typeof applyDamage === "function") applyDamage(targetElement, dmg); 
        } 
        if (typeof screenShake === "function") screenShake(); 
    } 
        ...
        if(targetElement.id.includes("hero")) { 
            if(targetElement.id === "player-hero") playerLife += heal; else enemyLife += heal; 
            if(window.VFX && window.VFX.triggerTechBits) VFX.triggerTechBits(targetElement, "#00ff00"); 
        } else { 
            targetElement.dataset.damageTaken = Math.max(0, (parseInt(targetElement.dataset.damageTaken) || 0) - heal); 
            if (typeof recalculateStats === "function") recalculateStats(targetElement); 
            if(window.VFX && window.VFX.triggerTechBits) VFX.triggerTechBits(targetElement, "#00ff00"); 
        } 
    } 
    else if (efeito === "atordoar" && !targetElement.id.includes("hero")) { 
        targetElement.classList.add("exhausted"); targetElement.dataset.hasAttacked = "true"; 
        if(window.VFX && window.VFX.stun) VFX.stun(targetElement); 
    }
    ...
    // =========================================================
    // ⚡ MAGIAS AVANÇADAS (COMPLEXAS)
    // =========================================================
    let ef = spellCard.dataset.effect || spellCard.dataset.originalEffect || "";

    // 📥 COMPRA DE CARTAS (Feitiço de Puxar)
    if (ef.includes("comprar_")) {
        let qtd = parseInt(ef.split("_")[1]) || 1;
        if (typeof comprarCartas === "function") comprarCartas(isLocalCaster ? "player" : "enemy", qtd);
        if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(document.getElementById(isLocalCaster ? "player-hero" : "enemy-hero"), `+${qtd} CARTAS`, "#00ffff");
    }

    // 🗑️ DESTRUIÇÃO DIRETA
    if (ef === "destruir_tropa" || ef === "assassinar") {
        if (targetElement && targetElement.classList.contains("card-base") && !targetElement.id.includes("hero")) {
            if (typeof destruirCartaNoCampo === "function") destruirCartaNoCampo(targetElement, targetOwner);
            if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(targetElement.parentElement, "ARQUIVO DELETADO", "#ff0000");
        } else {
            playSound("error"); alert("SISTEMA: Alvo inválido para deleção."); return;
        }
    }

    // 🛑 PROTOCOLO HAMAZ (Feitiço de Travar Slot)
    if (ef === "hamaz" || ef === "travar_slot") {
        let slotDOM = targetElement.classList.contains("slot") ? targetElement : targetElement.parentElement;
        if (slotDOM && slotDOM.classList.contains("slot")) {
            slotDOM.style.boxShadow = "inset 0 0 30px #00ffff"; 
            slotDOM.classList.add("slot-aprisionado");
            let cartaNoSlot = slotDOM.querySelector('.card-base');
            if (cartaNoSlot) {
                cartaNoSlot.dataset.hasAttacked = "true"; 
                cartaNoSlot.classList.add("exhausted"); 
                if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(cartaNoSlot, "SISTEMA TRAVADO", "#00ffff");
                if(window.VFX && window.VFX.stun) VFX.stun(cartaNoSlot);
            } else {
                if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(slotDOM, "SLOT CORROMPIDO", "#00ffff");
            }
        }
    }
    ...
    if (ef === "negacao_total") {
        ...
        field.querySelectorAll('.card-base').forEach(c => {
            if (c.dataset.raca === "mutante") {
                ...
                if (typeof recalculateStats === "function") recalculateStats(c);
                if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(c, "-2/-2 NEUTRALIZADO", "#00ffff");
            }
        });
    }
    ...
    if (ef === "esterilizacao_setor") {
        ...
        field.querySelectorAll('.card-base').forEach(c => {
            if (c.dataset.raca === "mutante") {
                ...
                if (typeof recalculateStats === "function") recalculateStats(c);
                if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(c, "PURGADO!", "#ff0000");
            }
        });
        if (typeof screenShake === "function") screenShake();
    }
    ...
    if (ef === "extracao_emergencia" && targetElement && !targetElement.id.includes("hero")) {
        let targetOwnerLocal = targetElement.parentElement.dataset.owner;
        if (typeof baseDeck !== "undefined") {
            let cardData = baseDeck.find(c => c.title === targetElement.dataset.name);
            if (cardData) {
                targetElement.remove(); // Tira da mesa
                if (targetOwnerLocal === "player") {
                    let novaCarta = createCard(cardData);
                    document.getElementById("hand").appendChild(novaCarta);
                    if (typeof arrangeHand === "function") arrangeHand();
                } else {
                    if (typeof enemyHand !== "undefined") enemyHand.push(cardData); // Devolve para a IA
                }
                if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(targetElement.parentElement, "EXTRAÍDO", "#00ffcc");
            }
        }
    }
    ...
    if (ef === "relatorio_classificado") {
        if (isPlayer && typeof enemyDeck !== "undefined" && enemyDeck.length >= 2) {
            ...
            if (typeof arrangeHand === "function") arrangeHand();
            if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(document.getElementById("player-hero"), "DADOS ROUBADOS", "#00ff00");
        }
    }
    ...
    if (ef === "varredura_amnesia") {
        if (isPlayer) {
            if (typeof enemyHand !== "undefined" && typeof enemyDeck !== "undefined") {
                if (enemyHand.length > 0) enemyDeck.push(enemyHand.pop());
                if (enemyHand.length > 0) enemyDeck.push(enemyHand.pop());
            }
            if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(document.getElementById("enemy-hero"), "-2 CARTAS", "#ff0055");
        } else {
            let mao = document.getElementById("hand");
            if (mao && mao.children.length > 0) mao.lastElementChild.remove();
            if (mao && mao.children.length > 0) mao.lastElementChild.remove();
            if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(document.getElementById("player-hero"), "MEMÓRIA APAGADA", "#ff0055");
            if (typeof arrangeHand === "function") arrangeHand();
        }
    }

    // =====================================================================
    // ⚡ GATILHO INTERCEPTADOR DE ANOMALIAS (Sniper)
    // =====================================================================
    ...
    aliados.forEach(aliado => {
        ...
        if (efeitoAliado.includes("interceptador") && aliado.dataset.dead !== "true") {
            ...
            if (inimigos.length > 0) {
                ...
                setTimeout(() => {
                    ...
                    if (typeof applyDamage === "function") applyDamage(alvoAzarado, 2);
                    if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(alvoAzarado, "INTERCEPTADO!", "#ffcc00");
                }, 500); 
            }
        }
    });
...
// =========================================================
// ⚙️ MOTOR DE EFEITOS DE TROPAS (Ao Jogar, Morrer, Fim de Turno)
// =========================================================
function processCardEffect(gatilho, cartaObj, owner) {
    ...
    // 🟢 GATILHO: AO JOGAR (ENTRAR EM CAMPO)
    if (gatilho === "AoJogar") {
        if (typeof registrarLog === "function") registrarLog(`Invocou: [${cartaObj.dataset.name}]`, owner);

        // ⚡ ROURKE (+2/+2 nos aliados)
        if (efeito === "buff_rourke" || cartaObj.dataset.name === "Rourke") {
            const aliados = document.getElementById(isPlayer ? "player-field" : "enemy-field").querySelectorAll('.card-base');
            aliados.forEach(aliado => {
                if (aliado !== cartaObj) {
                    aliado.dataset.baseAtk = (parseInt(aliado.dataset.baseAtk) || 0) + 2;
                    aliado.dataset.baseHp = (parseInt(aliado.dataset.baseHp) || 0) + 2;
                    if (typeof recalculateStats === "function") recalculateStats(aliado);
                    if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(aliado, "+2/+2 ROURKE", "#ffaa00");
                }
            });
            playSound("deploy");
        }
      
        // ⚡ OPERATIVO VÁCUO (Silêncio)
        if (efeito === "silencio_aleatorio") {
            ...
            if (alvos.length > 0) {
                ...
                alvo.dataset.effect = "nenhum"; 
                alvo.dataset.originalEffect = "nenhum";
                let typeBox = alvo.querySelector('.hud-type-box');
                if(typeBox) typeBox.innerText = "[SILENCIADO]";
                if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(alvo, "SISTEMA ANULADO!", "#555555");
                playSound("error");
            }
        }

        // ⚡ HAMAZ / HAZMAT (Aprisionar Slot + Stun)
        if (efeito === "aprisionar_slot" || cartaObj.dataset.name === "Hamaz") {
            const fieldEnemigo = document.getElementById(isPlayer ? "enemy-field" : "player-field");
            const slotsComInimigo = Array.from(fieldEnemigo.children).filter(slot => slot.querySelector('.card-base'));
            let slotAlvo = slotsComInimigo.length > 0 ? slotsComInimigo[Math.floor(Math.random() * slotsComInimigo.length)] : Array.from(fieldEnemigo.children)[Math.floor(Math.random() * 5)];

            if (slotAlvo) {
                slotAlvo.classList.add("slot-aprisionado");
                slotAlvo.style.boxShadow = "inset 0 0 20px #ffcc00"; 
                cartaObj.dataset.slotAprisionado = slotAlvo.id; 
                
                let vitima = slotAlvo.querySelector('.card-base');
                if (vitima) {
                    vitima.dataset.hasAttacked = "true";
                    vitima.classList.add("exhausted");
                    if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(vitima, "PRESO!", "#ffcc00");
                    if(window.VFX && window.VFX.stun) VFX.stun(vitima);
                } else {
                    if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(cartaObj, "ARMADILHA!", "#ffcc00");
                }
            }
        }
        ...
        if (efeito === "taxa_automato") {
            if (typeof mostrarTextoFlutuante === "function") mostrarTextoFlutuante(cartaObj, "CUSTO INIMIGO AUMENTADO", "#ff00ff");
        }
        ...
        if (efeito.includes("dano_area")) {
            ...
            for (let i = 0; i < campoInimigo.length; i++) {
                ...
                if (alvoDOM) {
                    if (typeof applyDamage === "function") applyDamage(alvoDOM, danoAoE);
        ...
        if (efeito.includes("Colmeia")) {
            ...
            if (temOutroEAO) {
                if (efeitosDisponiveis.length > 0) {
                    ...
                    if (typeof recalculateStats === "function") recalculateStats(cartaObj);
                }
        ...
        if (efeito.includes("extracao")) {
            ...
            if (aliados.length > 0) {
                ...
                if (isPlayer) {
                    if (typeof drawCard === "function") { drawCard(); setTimeout(drawCard, 300); }
                } 
        ...
        if (typeof falasPorTipo !== "undefined" && falasPorTipo[raca]) {
            let listaFalas = falasPorTipo[raca];
            let falaSorteada = listaFalas[Math.floor(Math.random() * listaFalas.length)];
            if (typeof projetarFalaHolografica === "function") projetarFalaHolografica(cartaObj, falaSorteada);
        }
        
        // ⚡ RECICLAR
        if (efeito === "reciclar" && isPlayer && graveyard.player.length > 0) { 
            let revivida = graveyard.player.pop(); 
            if (typeof updateLifeAndMana === "function") updateLifeAndMana(); 
        ...
        if (efeito === "roubo_energia" && isPlayer) { 
            enemyLife -= 1; 
            if (typeof updateLifeAndMana === "function") updateLifeAndMana(); 
            if(window.VFX && window.VFX.triggerTechBits) VFX.triggerTechBits(document.getElementById("enemy-hero"), "#ff00ff"); 
        }
        ...
        if (efeito === "tropa_coordenada") {
            ...
            if (emptySlots.length > 0) { 
                ...
                if (typeof recalculateStats === "function") recalculateStats(cartaObj); 
                if(window.VFX && window.VFX.pulse) VFX.pulse(cartaObj, "#00ff00"); playSound("hit"); 
            }
        }
        ...
        if (cartaObj.dataset.raca === "automato") {
            ...
            aliados.forEach(aliado => { 
                ...
                if (ef === "sinergia_automato" && aliado !== cartaObj) { 
                    ...
                    if (typeof recalculateStats === "function") recalculateStats(aliado); 
                    if(window.VFX && window.VFX.pulse) VFX.pulse(aliado, "#00ffff"); 
                } 
            });
        }
    }
    
    // 🔴 GATILHO: ÚLTIMO SUSPIRO (MORRER)
    if (gatilho === "UltimoSuspiro") { 
        if (typeof baseDeck !== "undefined" && typeof graveyard !== "undefined") {
            let cardData = baseDeck.find(c => c.title === cartaObj.dataset.name);
            if (cardData) graveyard[owner].push(cardData);
        }
        
        // ⚡ LIMPEZA HAZMAT
        if (cartaObj.dataset.slotAprisionado) {
        ...
        // ⚡ BLINDADO DE TRANSPORTE (Evocar Recruta)
        if (efeito === "evocar_recruta" || cartaObj.dataset.name === "Blindado de Transporte") { 
            const slot = cartaObj.parentElement; 
            setTimeout(() => { 
                if(slot && !slot.querySelector('.card-base')) { 
                    let recrutaData = baseDeck.find(c => c.title === "Cadete de Patrulha") || { title: "Cadete", atk: 1, hp: 1, type: "Tropa", cost: 1, rarity: "comum", efeito: "provocar" };
                    const recruit = createCard(recrutaData); 
        ...
        ["player-field", "enemy-field"].forEach(f => {
            ...
            if (field) {
                ...
                predadores.forEach(predador => {
                    ...
                    if (typeof recalculateStats === "function") recalculateStats(predador);
            ...
            for (let i = 0; i < campoAliado.length; i++) {
                ...
                if (alvoDOM) {
                    ...
                    if (danoAtual > 0) { 
                        ...
                        if (typeof recalculateStats === "function") recalculateStats(alvoDOM);
...
// =========================================================
// 🏆 SISTEMA DE GAME OVER (À PROVA DE FALHAS - ANTI CRASH)
// =========================================================
function checkGameOver() { 
    try {
        if (playerLife > 0 && enemyLife > 0) return; 
        if (window.gameIsOver === true) return; 
        ...
        console.log("[ZEUS] Fim de Partida. Vitória:", playerWon); // Deixa rastro para depuração
        
        const modal = document.getElementById("game-over-modal"); 
        
        // 🛡️ Se o modal não existir no HTML, avisa e reseta, mas não quebra o jogo!
        if (!modal) {
            console.error("[ERRO CRÍTICO] A div #game-over-modal não foi encontrada no HTML!");
            alert(playerWon ? "VITÓRIA! (Erro visual ao carregar prêmios)" : "SISTEMA CAIU!");
            location.reload();
            return;
        }

        let recompensaHTML = "";
        let playerFragments = parseInt(localStorage.getItem("zeusFragments")) || 0;
        ...
        if (playerWon) { 
            ...
            recompensaHTML = `<h2 style="color: #00ffff; margin: 15px 0;">+${ganho} 💽 HDs EXTRAÍDOS</h2>`; 
        }

        localStorage.setItem("zeusFragments", playerFragments); // Salva o dinheiro direto!

        // 🎨 RENDERIZA A TELA (Bem simples e blindado)
        ...
        modal.innerHTML = `
            <div class="modal-content final-screen" style="border-color: ${playerWon ? '#00ff00' : '#ff0000'}; background: rgba(5,5,10,0.95);">
                <h1 style="color: ${playerWon ? '#00ff00' : '#ff0000'}; margin-top: 15px;">
        ...
        // 🛡️ SE TUDO DER ERRADO, O JOGADOR VÊ ISSO (E O JOGO NÃO CONGELA)
        console.error("[CRASH DETECTADO NA PREMIAÇÃO]:", erro);
        alert("SISTEMA: Erro ao calcular recompensas. Fim de Simulação.");
        location.reload();
    }
}
